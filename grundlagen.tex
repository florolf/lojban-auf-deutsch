...
v & Wie das w in ``\bf{W}asser''\\
x & Wie das ch in ``Da\bf{ch}'' \\
y & Wie das e in ``M"uck\bf{e}'' oder das a in ``\bf{a}bout'' \\
z & Wie das s in ``\bf{S}uppe'' (stimmhaft) \\
\hline
\end{tabular}
...
\begin{quote}
\lojb{.o'i mu xagji sofybakni cu zvati le purdi}
\end{quote}
Was "ubersetzt ``Vorsicht! F"unf hungrige sovietische K"uhe sind im Garten''. (Das Wort \lojb{sofybakni} werden Sie "ofters sehen, jetzt wissen
Sie, woher es kommt)
...
Zu beachten ist jedoch, dass Namen auf Lojban (der Begriff hierf"ur ist \lojb{cmevla}) immer auf einen Konsonanten enden m"ussen.
...
In diesem Zusammenhang ist es auch gut, \lojb{coi} zu kennen, was ``Hallo'' bedeutet. (Wichtig ist, dass zwischen diesen \lojb{cmavo} und dem Namen immer eine Pause stehen muss)
...
Attitudinals tun genau das, was ihr Name Suggeriert: Sie sind eine M"oglichkeit den Gem"utszustand des Sprechers zu versprachlichen. Lojban hat eine F"ulle dieser Worte (Attitudinals sind auch \lojb{cmavo}) mit denen sich nahezu jede Kombination von Gef"uhlen ausdr"ucken l"asst.
...
%ist das hier angebracht?
\section{Weiteres zu \lojb{cmavo}}
Lojban lagert fast alle grammatikalischen Funktionen in \lojb{cmavo} aus, deshalb gibt es auch eine Unmenge von ihnen. Um die "Ubersicht behalten zu k"onnen werden sie ihrer Funktion entsprechend\footnote{Genaugenommen geht es hier nur um die grammatikalische Ebene, nicht die semantische, aber das nur am Rande.}
in verschiedene Klassen (auch \lojb{selma'o} genannt) eingeteilt. Die Namen sind "ublicherweise mehr oder weniger prominente Vertreter der Klasse, in Gro"sbuchstaben (ein ' wird durch ein ``h'' ersetzt).
Bisher sind wir den Klassen \lojb{COI} und \lojb{UI} begegnet. \lojb{UI} umfasst Attitudinals und Verwandte, \lojb{COI} Begr"u"sungen und "Ahnliches (so geh"ort \lojb{mi'e} auch dazu).
